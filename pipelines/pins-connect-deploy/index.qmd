...
Since we are using `vetiver_deploy_rsconnect()` which uses the {rsconnect} package, we need to make sure that we have the right authentication and settings. We can do this using `rsconnect::connectApiUser()`.
